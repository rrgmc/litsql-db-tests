...
func TestSakila2(t *testing.T) {
	...
	runDBTest(t, ctx, func(db *sql.DB) {
		query := psql.Select(
			...
			sm.WhereC("film.title = ? AND store.store_id = ?", "ACADEMY DINOSAUR", 1),
			sm.WhereC("NOT EXISTS ?", psql.Select(
				sm.Columns("*"),
				sm.From("rental"),
...
func TestSakila3(t *testing.T) {
	...
	runDBTest(t, ctx, func(db *sql.DB) {
		query := psql.Select(
			...
			sm.ColumnsC("rental_date + ? * INTERVAL '1 DAY' AS due_date", psql.Select(
				sm.Columns("rental_duration"),
				sm.From("film"),
				sm.WhereC("film_id = ?", 1),
			)),
			sm.From("rental"),
			sm.WhereC("rental_id = ?", psql.Select(
...
func TestSakila4(t *testing.T) {
	...
	runDBTest(t, ctx, func(db *sql.DB) {
		query := psql.Select(
			...
			sm.HavingC("avg(length) > ?", psql.Select(
...
func TestSakila7(t *testing.T) {
	...
	runDBTest(t, ctx, func(db *sql.DB) {
		query := psql.Select(
			...
			sm.From("actor"),
			sm.WhereC("actor_id IN ?", psql.Select(
				sm.Columns("actor_id"),
				sm.From("film_actor"),
				sm.WhereC("film_id IN ?", psql.Select(
					sm.Columns("film_id"),
					sm.From("film"),
...
func TestSakila8(t *testing.T) {
	...
	runDBTest(t, ctx, func(db *sql.DB) {
		query := psql.Select(
			...
			sm.From("cte"),
			sm.WhereC("cte.actor_count > ?", psql.Select(
